/**
 * Pricing validation utility for Cash on Delivery
 */

import { logger } from './logger';
import { handleValidationError } from './errorHandler';
import { productValidationService, ProductForValidation } from '../services/productValidationService';
 ...
 */
export const validateCartPricing = async (cartItems: CartItem[]): Promise<ValidationResult> => {
  const errors: string[] = [];
  ...
  const products = await productValidationService.getProductsByIds(productIds);
  ...
  for (const cartItem of cartItems) {
    ...
    const productValidation = productValidationService.validateProductPricing(product, cartItem.quantity);
    ...
    // Collect metadata
    if (product.hsn && !metadata.hsn.includes(product.hsn)) {
      metadata.hsn.push(product.hsn);
    }

    if (!metadata.categories.includes(product.category)) {
      metadata.categories.push(product.category);
    }

    if (!metadata.craftsmen.includes(product.craftsman)) {
      metadata.craftsmen.push(product.craftsman);
    }

    metadata.shippingWeight += (product.shippingWeight || 0) * cartItem.quantity;

    // Add warnings for low stock
    if (product.stock && product.stock <= 5) {
      warnings.push(`${product.name}: Only ${product.stock} items left in stock`);
    }

    // Add warnings for heavy items
    if ((product.shippingWeight || 0) > 30) {
 ...
 */
export const validateProductPricing = async (productId: string, quantity: number): Promise<ValidationResult> => {
  return await validateCartPricing([{ productId, quantity }]);
};
...
/**
 * Format pricing for COD order creation
 */
export const formatForCOD = (pricing: PricingBreakdown): {
  amount: number;
  currency: string;
  receipt: string;
  notes: Record<string, string>;
} => {
  const receipt = `cod_order_${Date.now()}_${Math.random().toString(36).substring(2, 8)}`;

  return {
    amount: pricing.summary.totalAmount,
 ...
 */
export const validateBeforePayment = async (
  cartItems: CartItem[],
  expectedAmount: number
): Promise<ValidationResult> => {
  const validation = await validateCartPricing(cartItems);
